...
| `getDeviceReportFreshness`, `getDeviceReportStaleAfterDays` | `_lib/device-health.js` | Workspace threshold resolution and row freshness classification |
...
2. Supports the following query parameters:
   ...
   - `manufacturer` and `policy_compliant` -- fleet facet and compliance filters
   - `report_freshness` -- `fresh`, `stale`, or `unknown`; evaluated against the workspace threshold independently of AMAPI state
...
5. Adds `report_freshness` to every device (`fresh`, `stale`, or `unknown`) and returns the effective `device_report_stale_after_days` threshold alongside the paginated response.
