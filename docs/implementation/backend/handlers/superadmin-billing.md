...
> Superadmin billing management: list/filter invoices, mark invoices as paid (with automatic license grant creation), cancel pending invoices, and create manual or gift seat grants.
...
|------|-------|-------------|
| `getRoute` | 23-42 | Parses the URL pathname tail to determine the sub-resource (`invoices`, `invoices/:id/mark-paid`, `invoices/:id/cancel`, or `grants/manual`) |
...
**Mark invoice paid (POST /invoices/:id/mark-paid):** Runs inside a transaction with `FOR UPDATE` row lock. Only pending invoices can transition to paid; cancelled invoices are rejected. The handler then iterates invoice items to create `license_grants` with source `invoice`. Each grant's seat count and duration are derived from item metadata.

**Cancel invoice (POST /invoices/:id/cancel):** Runs inside a transaction with `FOR UPDATE` row lock. Pending invoices transition to cancelled with an audit event and remain in invoice history. Repeated cancellation is idempotent. Paid invoices cannot be cancelled, and cancellation never creates or revokes grants.
...
| `POST` | `/.netlify/functions/superadmin-billing/invoices/:id/cancel` | Superadmin | Cancel a pending invoice without deleting its history |
