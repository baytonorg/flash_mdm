...
- **Access scope**: workspace membership mode that determines whether a user has workspace-wide visibility or is limited to explicit environment/group assignments.
...
- **Enterprise upgrade status**: cached AMAPI enterprise metadata indicating whether a bound managed Google Play Accounts enterprise can generate a Workspace upgrade URL.
...
- **Manual device re-import**: environment operation that scans AMAPI devices and enqueues local `process_enrollment` jobs for each discovered device name.
...
- **Outbound webhook**: a workflow/geofence side effect sent from Flash MDM to an operator-configured HTTPS URL after SSRF validation.
